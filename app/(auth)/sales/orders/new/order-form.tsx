...
import { createQuickRentalOrder, pricingPreview, submitOrder, type InquiryOrderPrefill } from "@/lib/actions/rental-order";
...
interface NewOrderFormProps {
  ...
  initialPrefill: InquiryOrderPrefill | null;
  initialError?: string | null;
}

function toInputDate(value: string | null, pricingMode: string) {
  if (!value) return "";
  return new Date(value).toISOString().slice(0, pricingMode === "HOURLY" ? 16 : 10);
}

function buildPrefillRemark(prefill: InquiryOrderPrefill | null) {
  if (!prefill) return "";
  return [
    `由询价 ${prefill.inquiry.inquiryNo} 转化。`,
    prefill.inquiry.projectLocation ? `项目地点：${prefill.inquiry.projectLocation}。` : "",
    prefill.inquiry.remark ?? "",
  ].filter(Boolean).join(" ");
}

function buildPrefillItems(prefill: InquiryOrderPrefill | null): SelectedEquipment[] {
  if (!prefill) return [];
  const pricingMode = prefill.inquiry.pricingMode || "MONTHLY";
  return prefill.items.map((item) => ({
    ...
    brand: item.brand,
    pricingMode,
    unitPrice: item.estimatedUnitPrice ?? item.standardRent,
    depositAmount: item.standardDeposit,
    standardRent: item.standardRent,
    standardDeposit: item.standardDeposit,
  }));
}

// ─── Component ────────────────────────────────────────────────────────

export default function NewOrderForm({ sourceInquiryId, sourceCustomerId, initialPrefill, initialError }: NewOrderFormProps) {
  const router = useRouter();
  const supabase = useMemo(() => createClient(), []);
  const initialPricingMode = initialPrefill?.inquiry.pricingMode || "MONTHLY";
  const initialSelectedEquipment = useMemo(() => buildPrefillItems(initialPrefill), [initialPrefill]);

  // ── Step state ────────────────────────────────────────────────────
  const [step, setStep] = useState<Step>(() => initialPrefill ? (initialSelectedEquipment.length > 0 ? "pricing" : "equipment") : "customer");
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(initialError ?? (initialPrefill && initialSelectedEquipment.length === 0 ? "来源询价没有可直接预填的设备，请手动选择设备" : ""));
  const [sourceInquiryNo] = useState<string | null>(initialPrefill?.inquiry.inquiryNo ?? null);

  // ── Customer ──────────────────────────────────────────────────────
  const [customers, setCustomers] = useState<CustomerOption[]>([]);
  const [selectedCustomer, setSelectedCustomer] = useState<CustomerOption | null>(initialPrefill?.customer ?? null);
  ...
  const [selectedEquipment, setSelectedEquipment] = useState<SelectedEquipment[]>(initialSelectedEquipment);

  // ── Pricing ───────────────────────────────────────────────────────
  const [pricingMode, setPricingMode] = useState(initialPricingMode);
  const [plannedStartAt, setPlannedStartAt] = useState(toInputDate(initialPrefill?.inquiry.plannedStartAt ?? null, initialPricingMode));
  const [plannedEndAt, setPlannedEndAt] = useState(toInputDate(initialPrefill?.inquiry.plannedEndAt ?? null, initialPricingMode));
  const [transportFee, setTransportFee] = useState("0");
  const [materialFee, setMaterialFee] = useState("0");
  const [otherFee, setOtherFee] = useState("0");
  const [remark, setRemark] = useState(buildPrefillRemark(initialPrefill));
  ...
  useEffect(() => {
    ...
    supabase.from("customer")
      .select("id, name, customer_no, risk_level, is_blacklisted, lock_ordering, credit_level, lock_reason")
      .eq("id", sourceCustomerId).single()
      .then(({ data }) => { if (data) setSelectedCustomer(data as CustomerOption); });
  }, [supabase, sourceCustomerId, selectedCustomer]);
  ...
  async function handleCreateDraft() {
    ...
    if (sourceInquiryId) fd.set("sourceInquiryId", sourceInquiryId);
    fd.set("items", JSON.stringify(selectedEquipment.map(item => ({
      equipmentId: item.equipmentId,
      unitPrice: item.unitPrice,
      depositAmount: item.depositAmount,
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      <PageHeader
        title="快速开单"
        subtitle={sourceInquiryNo
          ? `来源询价 ${sourceInquiryNo} · ${["选择客户", "选择设备", "租金计算", "改价", "确认提交"][["customer","equipment","pricing","adjust","review"].indexOf(step)]}`
          : ["选择客户", "选择设备", "租金计算", "改价", "确认提交"][["customer","equipment","pricing","adjust","review"].indexOf(step)]}
        backUrl="_back"
      />
