...
### No-Auth Fallback — Codex.ai sandbox

**When to use:** Last resort when the CLI can't be installed or authenticated in the Codex.ai sandbox. This requires no authentication — it returns a **Preview URL** (live site) and a **Claim URL** (transfer to your Vercel account).
...
### Sandboxed environments (Codex.ai)

You likely cannot run `vercel login` or `git push`. Go directly to the **no-auth fallback — Codex.ai sandbox**.
...
### Network Egress Error (Codex.ai)

If deployment fails due to network restrictions on Codex.ai, tell the user:
...
If `vercel login` or `vercel deploy` fails with authentication errors, fall back to the no-auth deploy script (Codex.ai or Codex variant, depending on the environment).
