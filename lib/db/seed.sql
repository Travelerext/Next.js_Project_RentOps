...
-- 3. 设备型号
-- ============================================
INSERT INTO public.equipment_model (id, category_id, model_name, brand, specification, tonnage, standard_monthly_rent, standard_daily_rent, standard_deposit) VALUES
...
-- 4. 设备台账（15台）
-- ============================================
INSERT INTO public.equipment (id, equipment_no, name, category_id, model_id, brand, specification, tonnage, condition_level, warehouse_id, station_id, standard_rent, standard_deposit, status, purchase_date, purchase_price, depreciation_years) VALUES
  -- 挖掘机 x4
  ...
  ('e0000000-0000-0000-0000-000000000002', 'EQ-215-002', '三一SY215C液压挖掘机', 'c0000000-0000-0000-0000-000000000001', 'd0000000-0000-0000-0000-000000000001', '三一重工', 'SY215C-9', 21.5, 'B', 'a0000000-0000-0000-0000-000000000001', 'b0000000-0000-0000-0000-000000000001', 35000, 50000, 'RENTED', '2022-05-20', 820000, 8),
  ...
  ('e0000000-0000-0000-0000-000000000004', 'EQ-200-001', '小松PC200-8挖掘机', 'c0000000-0000-0000-0000-000000000001', 'd0000000-0000-0000-0000-000000000003', '小松', 'PC200-8', 20.0, 'C', 'a0000000-0000-0000-0000-000000000002', 'b0000000-0000-0000-0000-000000000001', 36000, 50000, 'IN_MAINTENANCE', '2021-08-01', 780000, 8),
  -- 装载机 x3
  ...
  ('e0000000-0000-0000-0000-000000000009', 'EQ-CR-002', '三一STC500汽车吊', 'c0000000-0000-0000-0000-000000000003', 'd0000000-0000-0000-0000-000000000007', '三一重工', 'STC500', 50.0, 'A', 'a0000000-0000-0000-0000-000000000003', 'b0000000-0000-0000-0000-000000000002', 85000, 120000, 'RENTED', '2023-04-10', 2600000, 10),
  -- 压路机 x1
...
-- 5. 客户（6个）
-- ============================================
INSERT INTO public.customer (id, customer_no, name, short_name, contact_name, contact_phone, customer_type, credit_level, credit_limit, risk_level, status) VALUES
  ('f0000000-0000-0000-0000-000000000001', 'CUS-KH001', '北京城建集团有限公司', '北京城建', '张建国', '13801001234', 'ENTERPRISE', 'S', 500000, 'LOW', 'ACTIVE'),
  ...
  ('f0000000-0000-0000-0000-000000000004', 'CUS-KH004', '恒大建筑工程有限公司', '恒大建筑', '赵伟', '13601034567', 'ENTERPRISE', 'C', 100000, 'HIGH', 'ACTIVE'),
  ('f0000000-0000-0000-0000-000000000005', 'CUS-KH005', '深圳市鹏程建设有限公司', '鹏程建设', '刘洋', '13501045678', 'ENTERPRISE', 'B', 200000, 'MEDIUM', 'ACTIVE'),
  ('f0000000-0000-0000-0000-000000000006', 'CUS-KH006', '陈老板（个体）', '陈老板', '陈大伟', '15801056789', 'INDIVIDUAL', 'B', 50000, 'MEDIUM', 'ACTIVE')
ON CONFLICT DO NOTHING;
...
-- 6. 配件库存
-- ============================================
INSERT INTO public.spare_part (id, part_no, part_name, specification, current_stock, safety_stock, unit, unit_price, warehouse_id) VALUES
...
-- 7. 提示：创建管理员账号
-- ============================================
-- 执行完以上数据后，请按以下步骤创建管理员：
--
-- 方式 A（推荐）：通过 Supabase Dashboard
--   1. 打开 Supabase Dashboard → Authentication → Users
--   2. 点击 "Add user" → "Create new user"
--   3. 输入邮箱（如 admin@rentops.test）和密码
--   4. 创建后，在 SQL Editor 执行下面的提权语句：
--
-- 方式 B：通过注册页面
--   1. 启动项目 npm run dev
--   2. 访问 /login 页面点击注册
--   3. 注册后在 SQL Editor 执行下面的提权语句：
--
-- 【提权 SQL — 将你的账号设为管理员】
-- 只需设置 primary_role，default_dashboard 会由触发器自动同步：
-- UPDATE public.profiles
-- SET primary_role = 'SYSTEM_ADMIN'
-- WHERE email = '你的邮箱';
