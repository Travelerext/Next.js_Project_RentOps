-- Update trigger to also match by equipment_id
CREATE OR REPLACE FUNCTION public.trg_advance_return_on_inbound()
RETURNS TRIGGER AS $$
BEGIN
  UPDATE public.return_request
  SET request_status = 'PENDING_APPROVAL', inbound_id = NEW.id, updated_at = now()
...
CREATE TRIGGER trg_inbound_advance_return
  ...
  FOR EACH ROW EXECUTE FUNCTION public.trg_advance_return_on_inbound();

-- Fix currently stuck PENDING records
